...
let pp_entry_tab ppf entry =
  ...
  let pp_split_terse ppf split =
    let pp_print_split_pair ppf (user, v) = fprintf ppf "%s:%.2f" user.name v in
    match split with
    | Amt l ->
        fprintf ppf "($):[%a]"
...
type key = int
type t = { keygen : int Gen.gen; users : user list; tab : (int, entry) Table.t }

let create users = { keygen = Gen.init Fun.id; users; tab = Table.create () }

let add_expense t entry =
  let key = Gen.next t.keygen |> Option.get in
  Table.add t.tab key entry

let delete_expense t key = Table.delete t.tab key
let modify_expense t key new_entry = Table.modify t.tab key new_entry

let show t : unit =
  let open Format in
  set_margin 200;
  let headers = [ "added by"; "date"; "amt"; "split" ] in
  printf "%a" (Table.pp headers pp_print_int pp_entry_tab) t.tab
